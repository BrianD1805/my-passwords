...
-- Admin Customer Operations metadata. These tables contain Admin notes and
-- account-email delivery records only. They never contain vault ciphertext,
-- decrypted vault values, document contents or master passwords.
...
-- Automated customer lifecycle email delivery metadata.
-- Full email bodies and vault contents are not stored here.
...
-- Ver-0.049A Admin Automated Emails processor-run history.
...
create index if not exists idx_email_processor_runs_type_started on public.email_processor_runs(processor_type, started_at desc);
create index if not exists idx_email_processor_runs_status_started on public.email_processor_runs(status, started_at desc);
...
-- My Passwords Ver-0.050
-- Apply db/migrations/2026-08-07_security_hardening_ver_0_050.sql after this base schema.
...
-- Owner/Admin automatic email notifications and trial extension requests.
...
insert into public.admin_notification_settings (id, recipient_email, enabled, event_flags)
values ('owner_admin','bdh1805@gmail.com',true,jsonb_build_object('new_client_onboarded',true,'new_subscription_purchased',true,'trial_extension_requested',true,'payment_failed',true,'subscription_cancelled',true,'account_deletion_requested',true))
...
create unique index if not exists idx_trial_extension_requests_one_pending on public.trial_extension_requests(tenant_id) where status='pending';
...
-- Password-Encrypt Ver-1.020 — per-user settings
