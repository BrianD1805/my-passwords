...
check('Ver-1.002.02 version is aligned', pkg.version === '1.2.2' && /Password-Encrypt Ver-1\.002\.02/.test(main) && /my-passwords-v1\.002\.02/.test(read('public/sw.js')));
...
check('Released Emergency Package offers individual documents and one full ZIP containing TXT, DOCX and Documents folder', /downloadReleasedEmergencyDocument/.test(main) && /Download full ZIP/.test(main) && /Password-Encrypt-Emergency-Package\.zip/.test(main) && /Password-Encrypt-Emergency-Package\.txt/.test(main) && /Password-Encrypt-Emergency-Package\.docx/.test(main) && /Documents\/\$\{String\(index \+ 1\)/.test(main));
