...
check('App version is Ver-1.009', /Password-Encrypt Ver-1\.009/.test(main));
check('npm version is 1.9.0', pkg.version === '1.9.0');
check('Server version is Ver-1.009', /Password-Encrypt Ver-1\.009/.test(db));
check('Service worker cache is Ver-1.009', /my-passwords-v1\.009/.test(sw));
check('Offline page is Ver-1.009', /Password-Encrypt Ver-1\.009/.test(offline));
...
if (failed.length) {
  console.error(`\n${failed.length} Ver-1.009 feature check(s) failed.`);
  process.exit(1);
}
console.log(`\n${checks.length}/${checks.length} Ver-1.009 Admin Email checks passed.`);
