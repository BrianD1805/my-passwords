...
check('Ver-1.016 versions align', pkg.version === '1.16.0' && pkgLock.version === '1.16.0' && /Password-Encrypt Ver-1\.016/.test(main) && /Password-Encrypt Ver-1\.016/.test(db) && /my-passwords-v1\.016/.test(sw));
...
check('No database migration is required by this build', !fs.readdirSync(path.join(root, 'db/migrations')).some((name) => /1[_-]016/i.test(name)));
...
if (failures) {
  console.error(`\n${failures} Ver-1.016 check(s) failed.`);
  process.exit(1);
}
console.log(`\n${checks}/${checks} Ver-1.016 final onboarding flow checks passed.`);
