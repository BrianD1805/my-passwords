...
const adminDetail = read('src/AdminCustomerDetail.jsx');
const adminData = read('netlify/functions/admin-data.js');
const pkg = JSON.parse(read('package.json'));

const checks = [
  ['App version', main.includes('Password-Encrypt Ver-1.007.02')],
  ['npm version', pkg.version === '1.7.2'],
  ['Server version', db.includes('Password-Encrypt Ver-1.007.02')],
  ['Service-worker cache version', sw.includes('my-passwords-v1.007.02')],
  ['Offline version', offline.includes('Password-Encrypt Ver-1.007.02')],
  ...
  ['Admin customer data includes email and mobile verification state', adminData.includes('emailVerified: Boolean(primaryCustomerUser?.emailVerified)') && adminData.includes('phoneVerified: Boolean(primaryCustomerUser?.phoneVerified)')],
  ['Admin customer list shows email verification status', admin.includes("'Email verified' : 'Email not verified'")],
  ['Admin customer list shows mobile verification status', admin.includes("'Mobile verified' : 'Mobile not verified'") && admin.includes("'No mobile'")],
  ['Admin customer detail shows verified mobile state', adminDetail.includes("summary.phoneVerified ? 'Mobile verified' : 'Mobile not verified'")],
  ['Admin profile labels mobile as verified or not verified', adminDetail.includes("owner.phone_verified ? 'Verified' : 'Not verified'")]
];
...
console.log(`\n${pass}/${checks.length} Ver-1.007.02 SMS/Admin verification checks passed.`);
