...
check('Ver-1.024.02 app/server/package/service-worker versions align', pkg.version === '1.24.2' && /Password-Encrypt Ver-1\.024/.test(main) && /Password-Encrypt Ver-1\.024/.test(server) && /my-passwords-v1\.024/.test(sw));
...
check('Legal document version is aligned client and server for Emergency Backup Codes', /LEGAL_VERSION = '2026-08-26'/.test(legal) && /LEGAL_VERSION = '2026-08-26'/.test(bootstrap) && /LEGAL_EFFECTIVE_DATE = '26\/Aug\/2026'/.test(legal));
