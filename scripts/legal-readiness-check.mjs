...
check('Ver-0.054D app/server/package/service-worker versions align', pkg.version === '0.0.54-d' && /Password-Encrypt Ver-0\.054D/.test(main) && /Password-Encrypt Ver-0\.054D/.test(server) && /my-passwords-v0\.054D/.test(sw));
...
check('Legal document version is aligned client and server after the product rename', /LEGAL_VERSION = '2026-08-09'/.test(legal) && /LEGAL_VERSION = '2026-08-09'/.test(bootstrap) && /LEGAL_EFFECTIVE_DATE = '9 August 2026'/.test(legal));
