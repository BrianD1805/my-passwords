...
check('Ver-1.001.02 app/package/service-worker versions align', pkg.version === '1.1.2' && /Password-Encrypt Ver-1\.001\.02/.test(main) && /my-passwords-v1\.001\.02/.test(sw));
...
check('Master-password fields no longer request browser-generated new-password suggestions', /className="onboarding-secret-mask" type=\{onboardingSecretInputType\}[^>]*autoComplete="off"/.test(main) && !/vault-onboarding[^\n]{0,300}autoComplete="new-password"/.test(main));
check('Master-password fields retain password-manager ignore hints', /onboarding-secret-mask[^>]*data-lpignore="true"[^>]*data-1p-ignore="true"[^>]*data-form-type="other"/.test(main));
...
check('Vault-login key action blurs fields and never focuses the master-password keyboard', /async function handleBiometricIconAction\(event\)[\s\S]*masterPasswordInputRef\.current\?\.blur/.test(main) && !/handleBiometricIconAction[\s\S]{0,1200}focusMasterPassword\(\)/.test(main));
check('Secure device unlock setup is performed from Settings rather than the login key', /onClick=\{enableBiometricUnlock\}/.test(main) && /key icon on the login screen opens your device security prompt only/.test(main));
check('Step 2 clears stale Step 1 legal warnings during the route handoff', /setOnboardingSecretFieldsArmed\(\{ master: false, confirm: false \}\);[\s\S]*setMessage\(''\);[\s\S]*setToasts\(\[\]\);[\s\S]*window\.history\.replaceState/.test(main));
check('Onboarding master-password fields use browser-resistant masked text where supported', /onboardingSecretInputType/.test(main) && /className="onboarding-secret-mask" type=\{onboardingSecretInputType\}/.test(main) && /-webkit-text-security: disc/.test(css));
check('Account Setup and Vault Setup fields carry password-manager ignore metadata', /Display name<input autoComplete="off"[^>]*data-lpignore="true"/.test(main) && /Email address<input type="email" autoComplete="off"[^>]*data-lpignore="true"/.test(main) && /onboarding-secret-mask[^>]*data-lpignore="true"[^>]*data-1p-ignore="true"/.test(main));

if (failures) {
  console.error(`\n${failures} onboarding flow static check(s) failed.`);
  process.exit(1);
}
console.log('\nAll 52 onboarding flow static checks passed.');
