...
check('Ver-0.055b app/package/service-worker versions align', pkg.version === '0.0.55-b' && /Password-Encrypt Ver-0\.055b/.test(main) && /my-passwords-v0\.055b/.test(sw));
...
check('Step 2 wording no longer refers to a separate vault-setup screen', /Step 2 of 2 is next: create your encrypted vault and choose the master password only you know\./.test(main) && !/create your encrypted vault on a separate vault-setup screen/.test(main));
...
console.log('\nAll 24 onboarding flow static checks passed.');
