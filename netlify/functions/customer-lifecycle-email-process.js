...
async function retryFailedEmailRows(results) {
  ...
  for (const row of rows || []) {
    ...
    const delivery = await safeSend({
      ...
      metadata: { ...(row.metadata || {}), retry_source: 'scheduled_lifecycle_processor' }
    }, results);
  ...
  for (const tenant of tenants || []) {
    ...
    if (activeTrial && Number.isFinite(trialEndMs) && trialEndMs > nowMs && trialEndMs - nowMs <= threeDaysMs) {
      await safeSend({
        ...
        metadata: { source: 'scheduled_lifecycle_processor' }
      }, results);
    }
      ...
      if (subscription?.provider === 'stripe' && subscription?.provider_subscription_id) {
        ...
        if (refreshed?.row) {
          ...
          if (refreshedStatus === 'active') {
            await safeSend({ tenantId: tenant.id, userId: user.id, type: 'subscription_activated', idempotencyKey: `subscription_activated:${subscription.provider_subscription_id}:${subscription.current_period_start || subscription.updated_at || 'active'}`, metadata: { source: 'trial_end_reconciliation' } }, results);
            ...
            await safeSend({ tenantId: tenant.id, userId: user.id, type: 'payment_failed', idempotencyKey: `payment_failed:${subscription.id}:${failureKey}`, metadata: { source: 'trial_end_reconciliation' } }, results);
            if (subscription.grace_period_ends_at) await safeSend({ tenantId: tenant.id, userId: user.id, type: 'grace_period_started', idempotencyKey: `grace_period_started:${subscription.id}:${failureKey}`, metadata: { source: 'trial_end_reconciliation' } }, results);
          }
        }
      } else {
        ...
        await recordLifecycleEvent({ tenantId: tenant.id, subscriptionId: subscription?.id || null, eventType: 'trial_expired', status: 'recorded', metadata: { trial_ends_at: trialEndsAt, source: 'scheduled_lifecycle_processor' } }).catch(() => null);
        await safeSend({ tenantId: tenant.id, userId: user.id, type: 'trial_expired', idempotencyKey: `trial_expired:${tenant.id}:${trialEndsAt}`, context: { trialEndsAt }, metadata: { source: 'scheduled_lifecycle_processor' } }, results);
      }
    }
    ...
    if (subscription?.provider === 'stripe' && ['active', 'trialing'].includes(paidStatus) && subscription.cancel_at_period_end) {
      await safeSend({ tenantId: tenant.id, userId: user.id, type: 'cancellation_scheduled', idempotencyKey: `cancellation_scheduled:${subscription.id}:${subscription.current_period_end || 'period_end'}`, context: { cancellationAt: subscription.current_period_end }, metadata: { source: 'scheduled_lifecycle_processor' } }, results);
    }
    ...
    if (subscription?.provider === 'stripe' && ['past_due', 'unpaid'].includes(paidStatus) && subscription.last_payment_failed_at) {
      await safeSend({ tenantId: tenant.id, userId: user.id, type: 'payment_failed', idempotencyKey: `payment_failed:${subscription.id}:${subscription.last_payment_failed_at}`, metadata: { source: 'scheduled_lifecycle_processor' } }, results);
      if (subscription.grace_period_ends_at) await safeSend({ tenantId: tenant.id, userId: user.id, type: 'grace_period_started', idempotencyKey: `grace_period_started:${subscription.id}:${subscription.last_payment_failed_at}`, context: { gracePeriodEndsAt: subscription.grace_period_ends_at }, metadata: { source: 'scheduled_lifecycle_processor' } }, results);
    }
    ...
    if (subscription?.provider === 'stripe' && ['cancelled', 'canceled', 'incomplete_expired'].includes(paidStatus) && subscription.cancelled_at) {
      ...
      if (Number.isFinite(cancelledMs) && nowMs - cancelledMs <= 48 * 60 * 60 * 1000) {
        await safeSend({ tenantId: tenant.id, userId: user.id, type: 'subscription_cancelled', idempotencyKey: `subscription_cancelled:${subscription.id}:${subscription.cancelled_at}`, metadata: { source: 'scheduled_lifecycle_processor' } }, results);
      }
    }
  }

  await retryFailedEmailRows(results);

  const finishedAt = new Date().toISOString();
  console.log(JSON.stringify({ version: APP_VERSION, startedAt, finishedAt, tenantsChecked: tenants.length, emailActions: results.length, results }));
  return { statusCode: 200, body: JSON.stringify({ ok: true, version: APP_VERSION, startedAt, finishedAt, tenantsChecked: tenants.length, emailActions: results.length, results }) };
}
