...
export async function handler(event) {
  ...
  try {
    const plans = await selectRows('subscription_plans', 'select=code,display_name,description,currency,monthly_price_minor,quarterly_price_minor,annual_price_minor,trial_days,max_users,item_limit,storage_limit_mb,document_limit,features,feature_flags,is_featured,display_order,stripe_sync_status,stripe_monthly_price_id,stripe_quarterly_price_id,stripe_annual_price_id&is_public=eq.true&is_active=eq.true&order=display_order.asc,display_name.asc');
