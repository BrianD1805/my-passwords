...
export async function runOperationalRetentionCleanup({ triggerSource = 'scheduled' } = {}) {
  ...
  try {
    ...
    const [eventsDeleted, checksDeleted, reconciliationsDeleted, customerEmailsDeleted, adminEmailsDeleted, adminNotificationsDeleted, smsLogsDeleted, pushLogsDeleted, deletionRequestsDeleted, trialRequestsDeleted] = await Promise.all([
      ...
      deleteRows(`admin_notification_log?created_at=lt.${encodeURIComponent(metadataBefore)}`),
      ...
      deleteRows(`trial_extension_requests?status=in.(approved,declined,cancelled)&created_at=lt.${encodeURIComponent(deletionHistoryBefore)}`)
    ]);
    const total = eventsDeleted + checksDeleted + reconciliationsDeleted + customerEmailsDeleted + adminEmailsDeleted + adminNotificationsDeleted + smsLogsDeleted + pushLogsDeleted + deletionRequestsDeleted + trialRequestsDeleted;
    const summary = { eventsDeleted, checksDeleted, reconciliationsDeleted, customerEmailsDeleted, adminEmailsDeleted, adminNotificationsDeleted, smsLogsDeleted, pushLogsDeleted, deletionRequestsDeleted, trialRequestsDeleted };
