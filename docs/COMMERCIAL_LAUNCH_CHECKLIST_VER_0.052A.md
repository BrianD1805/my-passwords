# My Passwords Ver-0.052A — Commercial Launch Checklist
...
The public application currently identifies My Passwords as **a ZippyWeb project** and publishes **info@zippyweb.uk** for support and privacy requests.
...
## 2. Stripe statement descriptor — MUST CONFIRM BEFORE LAUNCH

Stripe's account statement descriptor is a merchant-account setting, not a My Passwords vault setting. Confirm it in the Stripe Dashboard before accepting public payments.
...
Recommended approach: use the shortest clear descriptor that genuinely matches the merchant identity customers know. Do **not** set `ZIPPYWEB` or `MY PASSWORDS` merely because it looks convenient; confirm it matches the Stripe account's verified business/DBA information first.
...
My Passwords displays Stripe-hosted invoice/receipt links when available. Stripe remains the authoritative payment record for Stripe charges.
...
Ver-0.052A deliberately does **not** claim that public prices are tax-inclusive. The public Billing Policy says tax depends on the seller's registrations and customer location.
...
When enabled, My Passwords passes `automatic_tax[enabled]=true` to Stripe Checkout and allows Checkout to update the existing Stripe customer's billing address so Stripe can use the checkout address for tax calculation.
...
Use Stripe invoice/payment IDs and My Passwords metadata for billing investigations.
