...
export const LEGAL_EFFECTIVE_DATE = '26/Aug/2026';
export const LEGAL_DISPLAY_VERSION = '26/Aug/2026';
...
function LegalShell({ title, eyebrow, intro, children, embedded = false }) {
  useEffect(() => {
    if (embedded) return;
    document.title = `${title} | Password-Encrypt`;
  ...
  const documentContent = (
    <>
      ...
      <section className="legal-contact-card">
        <Mail size={21} />
        <div><strong>Questions or privacy requests</strong><p>Password-Encrypt is a {BUSINESS_NAME} project. Contact <a href={`mailto:${SUPPORT_EMAIL}`}>{SUPPORT_EMAIL}</a>. Never send your master password, vault contents, recovery codes or secret keys to support.</p></div>
      </section>
    </>
  );
  ...
  return (
    <main className="legal-page-shell">
      <header className="legal-page-header">
        <a className="legal-back-link" href="/"><ArrowLeft size={18} /> Back to Password-Encrypt</a>
        <div className="legal-brand-lock"><Lock size={20} /></div>
        <div>
          <span className="legal-brand-name">Password-Encrypt</span>
          <small>A {BUSINESS_NAME} project</small>
        </div>
      </header>
      <article className="legal-document">{documentContent}</article>
      <footer className="legal-page-footer">
        <span>© 2026 Password-Encrypt · A {BUSINESS_NAME} project</span>
...
function TermsPage({ embedded = false }) {
  return (
    <LegalShell
      ...
      intro="These Terms explain the rules for using Password-Encrypt, including account access, encrypted vault services, Emergency Backup Codes, subscriptions and Emergency Access. Mandatory consumer rights that apply where you live are not excluded by these Terms."
      embedded={embedded}
    >
      <section><h2>1. Service and operator</h2><p>Password-Encrypt is a password-vault service operated as a {BUSINESS_NAME} project. The service includes a local encrypted vault, optional encrypted cloud backup and syncing, account verification, optional Emergency Backup Codes, subscription management, encrypted document storage where included in the plan, and Emergency Access where enabled.</p></section>
      ...
      <section><h2>3. Master password, encryption and recovery options</h2><div className="legal-highlight"><ShieldCheck size={21} /><div><strong>Client-side encrypted vault</strong><p>Vault records and uploaded document contents are encrypted in the browser before backup or upload. The current implementation uses AES-GCM with a key derived from the master password using PBKDF2-SHA-256. Password-Encrypt does not intentionally send or store the master password on its servers.</p></div></div><p>Because Password-Encrypt does not hold a readable server-side copy of the master password, support cannot recover or reset it and cannot normally decrypt stored vault snapshots or encrypted document contents. If you deliberately generate Emergency Backup Codes while the vault is unlocked, the browser creates separately encrypted recovery envelopes that can later recover the protected master-password copy when you supply one of your high-entropy codes after account-email verification. After a successful backup-code recovery, Password-Encrypt requires you to set a new master password before continuing, re-encrypts the current vault under that new password, invalidates the previous recovery-code set and prepares a fresh set for you to save. Password-Encrypt stores encrypted recovery envelopes and one-way code fingerprints, not the readable codes. A working Secure device unlock on a device you already configured may also provide local access, and Emergency Access may release information that was prepared in advance; none of these mechanisms gives support a server-side password-reset capability.</p><p>If you deliberately enable Secure device unlock, Password-Encrypt keeps a wrapped copy of the master password and a non-exportable local device key in that browser/device storage so the password can be recovered locally after the platform credential check. This convenience feature changes the local-device threat model: anyone or any software that compromises the browser, device or same-origin application context may be able to attack that locally stored material. Secure device unlock can be removed from that device and periodically requires the master password again.</p><p>No online service can promise absolute security. Protection also depends on the integrity of your browser, device, operating system, network and the application code delivered to your device. You should keep devices updated and protect them against malware, unauthorised access and account compromise.</p></section>
      <section><h2>4. Free trials</h2><p>A free trial starts only after the new account successfully completes the contact-verification step. Creating a trial account does not by itself create a paid subscription and does not require a card. A paid subscription begins only when you deliberately complete Stripe Checkout.</p><p>The trial end date is shown in Password-Encrypt. If you start a subscription during the trial, Stripe Checkout shows the applicable price and the expected first charge or renewal timing before you confirm payment. Starting a paid subscription does not guarantee an extension of an existing trial.</p></section>
      <section><h2>5. Paid subscriptions and renewals</h2><p>Subscriptions are billed in the currency and billing period shown before checkout. Unless you cancel, a subscription renews automatically at the end of each billing period using the payment method managed through Stripe.</p><p>Higher-plan upgrades may take effect immediately and Stripe may apply a prorated billing adjustment. Downgrades and billing-period changes are normally scheduled for the next renewal. Only plans actually published in Password-Encrypt are available for purchase.</p></section>
      <section><h2>6. Cancellation</h2><p>You may schedule cancellation from Plan & Billing. Cancellation takes effect at the end of the current paid billing period unless applicable law requires a different result. Access included in the paid subscription remains available until that period ends, subject to payment status and these Terms. A scheduled cancellation can be removed before the subscription ends.</p><p>Deleting an account is a separate action. If a paid Stripe subscription is still active when an account deletion becomes due, Password-Encrypt attempts to cancel that subscription before deleting the cloud account data.</p></section>
      ...
      <section><h2>8. Emergency Access</h2><div className="legal-highlight"><UsersRound size={21} /><div><strong>Emergency Access is an intentional disclosure mechanism</strong><p>If you enable it, you choose a trusted person, waiting period and release scope. You are notified when access is requested and can cancel during the waiting period. While the arrangement remains active, Password-Encrypt can refresh the prepared emergency package from the unlocked vault when included information changes or the vault reconnects online. If you do not cancel in time, the latest successfully prepared package is frozen as the release snapshot and can become available through the trusted person's secure link.</p></div></div><p>Emergency Access does not send your master password. The prepared package is separately encrypted for the emergency link. Anyone who gains control of that link may be able to use it according to the Emergency Access state, so the trusted person must keep it private. Once a package becomes available, its secure release link is available for 30 days. The trusted person may download the released package as a readable TXT or DOCX file; downloaded copies are no longer protected by Password-Encrypt's secure release link and must be stored privately by the recipient. If the trusted person already has a Password-Encrypt vault, they may instead choose to import the released package into their own encrypted vault as a separate received Emergency Package folder; that imported copy then remains part of their vault until they delete it. When Full vault access is selected, stored documents can also be decrypted locally in the owner's unlocked browser, re-encrypted specifically for the Trusted Person link, and maintained as part of the prepared package before release and included as downloadable document copies after the waiting period completes. Because Password-Encrypt cannot decrypt the vault on its servers, an offline or locked vault cannot be refreshed until the owner next opens it online. Once the waiting period completes, the released snapshot is fixed and later vault changes are not automatically shared with the trusted person.</p></section>
      <section><h2>9. Acceptable use</h2><p>You must not use Password-Encrypt to attack the service, interfere with other customers, bypass plan limits or security controls, distribute malware, or store or transmit content where doing so is unlawful. We may suspend cloud account services where reasonably necessary to protect customers, investigate abuse, comply with law or address serious payment or security problems.</p></section>
      <section><h2>10. Availability and backups</h2><p>We aim to operate the service reliably, but uninterrupted availability is not guaranteed. A successful local save is not the same as a completed cloud backup. Password-Encrypt displays sync and backup status so you can identify changes that still need protection. You remain responsible for responding to warnings and keeping devices reasonably protected.</p></section>
      <section><h2>11. Account deletion</h2><p>Account deletion requires verification and currently uses a 14-day safety period. You can cancel the deletion request during that period. When deletion completes, the Password-Encrypt cloud account, encrypted vault snapshots and stored encrypted documents linked to the account are removed from the active application database. Limited payment, legal, security or provider records may remain where they must be kept for legitimate legal, accounting, fraud-prevention or dispute purposes.</p></section>
      <section><h2>12. Third-party services</h2><p>Password-Encrypt relies on specialist providers for hosting, database services, payments and communications. Stripe processes payment-card information; Password-Encrypt does not store your full card details. Provider availability and their own terms can affect related features.</p></section>
...
function PrivacyPage({ embedded = false }) {
  return (
    <LegalShell
      ...
      intro="This Policy explains what personal data Password-Encrypt processes, why it is needed, what is encrypted, which service providers are involved, and how to request access, correction or deletion."
      embedded={embedded}
    >
      <section><h2>1. Who handles your data</h2><p>Password-Encrypt is a {BUSINESS_NAME} project. For account, support and service-administration data, {BUSINESS_NAME} acts as the service operator. Contact privacy and support requests at <a href={`mailto:${SUPPORT_EMAIL}`}>{SUPPORT_EMAIL}</a>.</p></section>
      <section><h2>2. Data we process</h2><div className="legal-data-grid"><div><strong>Account data</strong><p>Name, email, mobile number, account/vault name, verification state, plan and account status.</p></div><div><strong>Device and security metadata</strong><p>Verified-device identifiers, browser/platform information, session dates, security events and masked delivery destinations.</p></div><div><strong>Billing metadata</strong><p>Stripe customer/subscription identifiers, plan, billing period, amounts, invoice references and payment status. Full card details are handled by Stripe.</p></div><div><strong>Operational metadata</strong><p>Sync status, backup status, item/document counts, timestamps, error codes and service-health events designed not to contain decrypted vault contents.</p></div><div><strong>Communications</strong><p>Email/SMS delivery status and support information you choose to send. Do not send support your master password or vault contents.</p></div><div><strong>Emergency Backup Code data</strong><p>Encrypted recovery envelopes, one-way code fingerprints, generation/use status and reminder dates. Readable codes are not stored after generation; if you explicitly request email delivery of a newly generated set, the codes are processed transiently by the email service for that delivery.</p></div><div><strong>Emergency Access metadata</strong><p>Trusted-person contact details, waiting period, request/release status, 30-day release expiry and the separately encrypted owner-prepared emergency package. If the trusted person downloads a released package, that readable local file is controlled by the recipient rather than Password-Encrypt.</p></div></div></section>
      <section><h2>3. Encrypted vault data</h2><div className="legal-highlight"><Lock size={21} /><div><strong>Readable vault content is not the normal server-side data model</strong><p>Vault records and uploaded document contents are encrypted in your browser before they are sent for cloud storage. Server storage receives encrypted blobs plus the metadata needed to store, sync and enforce plan limits. The master password is not intentionally sent to or stored on Password-Encrypt servers.</p></div></div><p>We therefore avoid an absolute “zero knowledge” or “cannot ever be decrypted” claim. Client-side encryption materially limits what routine server systems and support tooling can read, but security can still be affected by your device, browser, malicious software, compromised links, future code changes, lawful requirements, or other circumstances outside the mathematical encryption itself.</p><p>If you enable Secure device unlock, a wrapped copy of the master password and the local cryptographic key used to unwrap it remain on that browser/device. If you generate Emergency Backup Codes, separately encrypted master-password recovery envelopes and one-way code fingerprints are stored with your account; the readable codes are not retained by Password-Encrypt after generation. When a backup code is used to recover a forgotten master password, the app requires a replacement master password, re-encrypts the current vault under that new password, invalidates the previous recovery-code set and removes older encrypted recovery points where possible before creating a fresh recovery-code set. If you explicitly ask to email a newly generated set, those readable codes are transmitted through the email-delivery provider for that requested delivery and the email copy is then under the security of your mailbox. These recovery materials are security-sensitive and should be protected accordingly.</p></section>
      ...
      <section><h2>10. Essential cookies and browser storage</h2><p>Password-Encrypt uses security/session cookies and browser storage that are necessary to provide login/session protection, the encrypted local vault, device identification, sync safety, CSRF protection, settings and optional Secure device unlock. Password-Encrypt does not intentionally include advertising or behavioural-analytics trackers in the service described by this Policy. Clearing browser/site data can remove the local encrypted vault, device verification state or Secure device unlock material from that browser.</p></section>
...
function BillingTermsPage({ embedded = false }) {
  return (
    <LegalShell
      ...
      intro="This Policy explains trials, recurring billing, subscription changes, cancellation, refunds, taxes, invoices and payment records for Password-Encrypt."
      embedded={embedded}
    >
      ...
      <section><h2>7. Failed payments</h2><p>If a renewal payment fails, Stripe may retry the payment according to the configured billing settings. Password-Encrypt may provide a grace period before paid cloud services are paused. The encrypted local vault remains separate from payment processing and still requires the master password.</p></section>
      <section><h2>8. Invoices and receipts</h2><p>Stripe generates and hosts subscription invoices and payment receipts. Password-Encrypt displays available links to those Stripe records in Plan & Billing. Stripe records can include billing address, line items, taxes, payment status and receipt information. Customers should keep invoices needed for their own accounting or tax records.</p></section>
